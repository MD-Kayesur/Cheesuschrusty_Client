...
// // --- Constants and Types (Kept consistent with previous update) ---
...
//   { id: '2', title: 'Italian Adjectives', description: 'Common descriptive words for people and things.', cards: 40, difficulty: 'Intermediate', category: 'Writing', status: 'Drafted', lastModified: '15/10/2025' },
//   { id: '3', title: 'Verb Conjugation', description: 'Basic present tense -are verbs.', cards: 30, difficulty: 'Advance', category: 'Speaking', status: 'Published', lastModified: '01/08/2025' },
// ];
...
//   const handleAdd = () => {
//     // We only require title and difficulty/category/status are set by default
//     if (!formData.title || formData.cards <= 0) {
//       toast.error('Deck Title, Description, and Cards ( > 0) are required.');
//       return;
//     }
...
//       {/* Dynamic Add/Edit/View Modal (UPDATED) */}
//       {(modalType === 'add' || modalType === 'edit' || modalType === 'view') && (
//         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
//           <div className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-md max-h-[95vh] overflow-y-auto relative">
...
//             {/* Add/Edit Mode Form (UPDATED) */}
//             {(modalType === 'add' || modalType === 'edit') && (
//               <div className="flex flex-col gap-5">
//                 {/* Header Section */}
//                 <div>
//                   <h3 className="text-2xl font-bold text-gray-800">
//                     {modalType === 'add' ? 'Create New Flashcard Deck' : 'Edit Flashcard Deck'}
//                   </h3>
//                   <p className="text-sm text-gray-500 mt-1">
//                     {modalType === 'add' 
//                       ? 'Add a new flashcard deck to your Italian learning content' 
//                       : `Update the details for ${selectedDeck?.title}`
//                     }
//                   </p>
//                 </div>
...
//                     <label htmlFor="title" className="text-sm font-medium text-gray-700 mb-1">Deck Title</label>
//                     <input
//                       id="title"
//                       placeholder="Enter deck title"
//                       value={formData.title}
//                       onChange={(e) => setFormData({ ...formData, title: e.target.value })}
//                       className="rounded border border-gray-300 p-3 focus:ring-blue-500 focus:border-blue-500 transition duration-150"
//                     />
//                 </div>
...
//                       className="rounded border border-gray-300 p-3 focus:ring-blue-500 focus:border-blue-500 transition duration-150 bg-white"
//                     >
//                       <option value="" disabled>Select</option> {/* Added Select placeholder */}
...
//                 {/* Hidden/Collapsed Fields (To keep data integrity for table) */}
//                 {/* Note: In a real app, these would likely be shown in a secondary/collapsed section or a multi-step form. 
//                     I'm keeping them hidden to strictly match the small modal image provided. */}
//                 <div className='hidden'>
//                   <input type="number" value={formData.cards} onChange={(e) => setFormData({ ...formData, cards: Math.max(0, Number(e.target.value)) })} />
//                   <select value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value as ContentItem['category'] })} >
//                     {CATEGORY_OPTIONS.map(option => (<option key={option} value={option}>{option}</option>))}
//                   </select>
//                   <select value={formData.status} onChange={(e) => setFormData({ ...formData, status: e.target.value as ContentItem['status'] })} >
...
//                   {/* The image only shows a single blue button. We'll use the blue button for the main action
//                       and rely on the 'X' button to close/cancel. For the Edit modal, we'll keep the Cancel button. */}
                  
//                   {modalType === 'edit' && (
//                     <button onClick={closeModal} className="rounded-lg bg-gray-200 px-5 py-2 text-gray-700 hover:bg-gray-300 transition-colors mr-3">
//                         Cancel
//                     </button>
//                   )}
...
//                     className="bg-blue-600 hover:bg-blue-700 rounded-lg px-8 py-3 text-white font-medium transition-colors w-full"
//                   >
