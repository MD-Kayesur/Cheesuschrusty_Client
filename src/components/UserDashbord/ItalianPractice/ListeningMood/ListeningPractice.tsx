...
import SentenceOrdering from './SentenceOrdering'; // Import the new component
...
const ListeningPractice: React.FC = () => {
  ...
  const [phase, setPhase] = useState<'questions' | 'dictation' | 'sentence'>('questions'); // Track which section to show
  ...
  const handleNext = () => {
    if (currentQuestion < questions.length - 1) {
      ...
      setPhase('dictation'); // Move to dictation after last question
    }
  };
  ...
  const handleDictationContinue = () => {
    setPhase('sentence'); // Move to SentenceOrdering
  };
          ...
          <div className="flex items-start justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Listen while answering</p>
              </div>
            </div>
          </div>
           ...
           <div className="bg-gray-50 rounded-lg p-6">
            <input
              ...
              value={currentTime}
              onChange={handleSeek}
              className="w-full h-1 bg-gray-300 rounded-lg appearance-none cursor-pointer mb-2"
              style={{ background: `linear-gradient(to right, #d1d5db 0%, #d1d5db ${(currentTime/duration)*100}%, #e5e7eb ${(currentTime/duration)*100}%, #e5e7eb 100%)` }}
            />
            ...
            <div className="flex items-center justify-center gap-4">
              <button className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-gray-200"><SkipBack className="w-5 h-5 text-gray-700" /></button>
              <button onClick={handlePlayPause} className="w-14 h-14 bg-indigo-600 hover:bg-indigo-700 rounded-full flex items-center justify-center shadow-lg">
                {isPlaying ? <Pause className="w-6 h-6 text-white" /> : <Play className="w-6 h-6 text-white ml-1" />}
              </button>
              <button className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-gray-200"><SkipForward className="w-5 h-5 text-gray-700" /></button>
            </div>
          </div>
        </div> */}
...
{/* Audio Player */}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div className="mb-4">
              ...
              <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                <div className="h-full bg-gray-900 transition-all duration-300" style={{ width: `${((currentQuestion + 1)/totalQuestions)*100}%` }} />
              </div>
            </div>
              ...
              {questions[currentQuestion].options.map((option, idx) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 border-2 rounded-lg transition-colors ${
                    questions[currentQuestion].selectedAnswer === option ? 'border-indigo-600 bg-indigo-50' : 'bg-white border-gray-200 hover:border-indigo-600 hover:bg-indigo-50'
                  }`}
                >
            ...
            <div className="flex justify-center gap-3">
              <button onClick={handlePrevious} disabled={currentQuestion===0} className={`px-6 py-3 border-2 rounded-lg flex items-center gap-2 ${currentQuestion===0?'border-gray-200 text-gray-400 cursor-not-allowed':'border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
                <ChevronLeft className="w-4 h-4" /> Previous
              </button>
              <button onClick={handleNext} disabled={!questions[currentQuestion].selectedAnswer} className={`px-6 py-3 rounded-lg flex items-center gap-2 ${!questions[currentQuestion].selectedAnswer?'bg-gray-300 text-gray-500 cursor-not-allowed':'bg-blue-600 text-white hover:bg-blue-700'}`}>
                {currentQuestion === totalQuestions-1 ? 'Finish' : 'Next'} <ChevronLeft className="w-4 h-4 rotate-180" />
              </button>
            </div>
          </div>
        )}
        ...
        {phase === 'sentence' && <SentenceOrdering />}
      </div>
    </div>
  );
};
